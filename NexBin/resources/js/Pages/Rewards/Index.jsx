...
function Toast({ toasts }) {
  return (
    <div className="fixed top-4 left-4 right-4 sm:left-auto sm:right-4 sm:w-auto z-[100] flex flex-col gap-2 pointer-events-none">
      {toasts.map((t) => (
        <div
          key={t.id}
          className={`flex items-center gap-2 px-4 py-3.5 rounded-2xl shadow-xl text-sm font-semibold text-white pointer-events-auto ${
  ...
  return (
    <div
      ...
      className={`group relative bg-white rounded-2xl overflow-hidden shadow-sm border transition-all duration-200 cursor-pointer
        active:scale-[0.98] hover:-translate-y-1 hover:shadow-lg
    ...
    >
      ...
      {/* Image */}
      <div className="relative overflow-hidden h-40 sm:h-36">
        <img
      ...
      <div className="p-4">
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          <button
            ...
            className={`px-4 py-2.5 rounded-xl text-xs font-bold transition-all duration-150 active:scale-95 whitespace-nowrap
              ${canRedeem && !redeeming
                ? "bg-[#1B1F5E] text-white hover:bg-[#2d3494] shadow-sm"
                : "bg-slate-100 text-slate-400 cursor-not-allowed"
              }`}
          >
            {outOfStock ? "Out of Stock" : needMore ? "Locked" : "Redeem"}
          </button>
        </div>
      </div>
    </div>
      ...
      >
        ...
        <div className="overflow-y-auto flex-1">
          ...
          <div className="relative h-52 flex-shrink-0">
            ...
            <button
              onClick={onClose}
              className="absolute top-4 right-4 w-10 h-10 bg-white/90 hover:bg-white rounded-full flex items-center justify-center text-slate-600 shadow transition-all active:scale-95"
            >
              ✕
            </button>
        ...
        <div className="p-4 border-t border-slate-100 bg-white flex-shrink-0 safe-bottom">
          <button
            disabled={!canRedeem || redeeming}
            onClick={() => onRedeem(reward)}
            className={`w-full py-4 rounded-2xl font-bold text-sm transition-all duration-150 active:scale-[0.98]
              ${canRedeem && !redeeming
    ...
    >
      ...
      >
        ...
        <div className="px-6 pt-2 pb-6 text-center">
          <div className="w-16 h-16 bg-[#1B1F5E]/10 rounded-full flex items-center justify-center mx-auto mb-4">
          ...
          <div className="flex gap-3">
            <button
              onClick={onCancel}
              disabled={redeeming}
              className="flex-1 py-3.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-2xl text-sm font-semibold transition-all active:scale-[0.97]"
            >
              Cancel
            </button>
            <button
              onClick={onConfirm}
              disabled={redeeming}
              className="flex-1 py-3.5 bg-[#1B1F5E] text-white rounded-2xl text-sm font-bold hover:bg-[#2d3494] transition-all active:scale-[0.97] disabled:opacity-60"
            >
              {redeeming ? (
                <span className="flex items-center justify-center gap-2">
                  <svg className="animate-spin w-4 h-4" viewBox="0 0 24 24" fill="none">
...
function ReceiptModal({ receipt, onClose, onViewHistory }) {
  ...
  return (
    <div
      className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-end sm:items-center justify-center z-[70]"
      style={{ animation: "fadeIn 0.2s ease" }}
    >
      <div
        ...
        <div className="p-5 space-y-4">
          <div className="border border-dashed border-slate-200 rounded-xl p-4 space-y-3">
            {[
              ...
              ["Account",        <span key="acct" className="text-slate-700 truncate max-w-[160px] inline-block">{receipt.userEmail}</span>],
              ["Points spent",   <span key="spent" className="font-bold text-[#1B1F5E]">−{receipt.pointsUsed.toLocaleString()} pts</span>],
              ["Remaining",      <span key="bal" className="font-bold text-slate-800">{receipt.pointsAfter.toLocaleString()} pts</span>],
            ].map(([label, val]) => (
          ...
          <div className="bg-emerald-50 rounded-xl px-4 py-3 flex gap-2.5 items-start">
            ...
            <p className="text-xs text-emerald-700 leading-relaxed">
              Your reward is successfully redeemed. Kindly proceed to the redemption location and present this receipt.
            </p>
          </div>
          ...
          <div className="flex gap-2 pt-1 pb-safe">
            <button
              ...
              className="flex-1 py-3.5 rounded-2xl border border-slate-200 text-xs font-semibold text-slate-600 hover:bg-slate-50 transition-all active:scale-[0.97]"
            >
              View History
            </button>
            <button
              onClick={onClose}
              className="flex-1 py-3.5 rounded-2xl bg-[#1B1F5E] text-white text-xs font-bold hover:bg-[#2d3494] transition-all active:scale-[0.98]"
            >
              Done
            </button>
          </div>
    ...
    >
      ...
      >
        ...
        <div className="bg-[#1B1F5E] px-6 py-5 text-center relative">
          <button
            onClick={onClose}
            className="absolute top-4 right-4 w-10 h-10 bg-white/20 hover:bg-white/30 rounded-full flex items-center justify-center text-white text-sm transition-all active:scale-95"
          >
            ✕
          </button>
        ...
        <div className="p-5 space-y-4">
          ...
          {redemption.status === "Completed" && (
            <div className="bg-emerald-50 rounded-xl px-4 py-3 flex gap-2.5 items-start">
              <span className="text-base mt-0.5 flex-shrink-0">✅</span>
              <p className="text-xs text-emerald-700 leading-relaxed">This reward has been fulfilled. Thank you!</p>
            </div>
          )}
          ...
          {!["Pending", "Completed", "Cancelled"].includes(redemption.status) && (
            <div className="bg-slate-50 rounded-xl px-4 py-3 flex gap-2.5 items-start">
              ...
              <p className="text-xs text-slate-600 leading-relaxed">
                Present this receipt to our partners at redemption location.
              </p>
            </div>
          )}
          ...
          <button
            onClick={onClose}
            className="w-full py-3.5 rounded-2xl bg-[#1B1F5E] text-white text-sm font-bold hover:bg-[#2d3494] transition-all active:scale-[0.98]"
          >
            Close
          </button>
        </div>
...
function Pagination({ currentPage, lastPage, onPageChange }) {
  ...
  return (
    <div className="flex items-center justify-between mt-6 pt-5 border-t border-slate-100 gap-2">
      <button
      ...
      <div className="flex items-center gap-1">
        {getPages().map((p, i) =>
          p === "..." ? (
            <span key={`ellipsis-${i}`} className="w-8 text-center text-xs text-slate-400 font-semibold select-none">···</span>
          ) : (
            <button
              key={p}
              onClick={() => onPageChange(p)}
              className={`w-10 h-10 rounded-xl text-xs font-bold transition-all active:scale-[0.95]
                ${p === currentPage
...
function RedemptionHistory({ redemptions = [], onView }) {
  ...
  return (
    <>
      ...
      {/* Mobile cards — tap-friendly, larger targets */}
      <div className="md:hidden space-y-3">
        {redemptions.map((r) => (
          <div
            key={r.id}
            onClick={() => onView(r)}
            className="bg-white rounded-2xl border border-slate-100 p-4 shadow-sm cursor-pointer
                       hover:border-[#1B1F5E]/30 hover:shadow-md transition-all active:scale-[0.99]"
          >
            <div className="flex items-start justify-between mb-3 gap-2">
              <div className="flex-1 min-w-0">
                <p className="font-bold text-sm text-slate-800 truncate">{r.reward?.name ?? "—"}</p>
...
function BottomNav({ activeSection, setActiveSection, redeemableCount }) {
  return (
    <div className="md:hidden fixed bottom-0 left-0 right-0 z-30 bg-white border-t border-slate-200 safe-bottom">
      <div className="flex">
        <button
          onClick={() => setActiveSection("rewards")}
          className={`flex-1 flex flex-col items-center justify-center py-3 gap-0.5 transition-colors ${
            activeSection === "rewards" ? "text-[#1B1F5E]" : "text-slate-400"
          }`}
        >
          <span className="text-xl">🎁</span>
          <span className="text-[10px] font-bold">Rewards</span>
          {redeemableCount > 0 && (
            <span className="absolute top-2 ml-6 bg-emerald-500 text-white text-[9px] font-bold w-4 h-4 rounded-full flex items-center justify-center">
              {redeemableCount > 9 ? "9+" : redeemableCount}
            </span>
          )}
        </button>
        <button
          ...
          className={`flex-1 flex flex-col items-center justify-center py-3 gap-0.5 transition-colors ${
            activeSection === "history" ? "text-[#1B1F5E]" : "text-slate-400"
          }`}
        >
          <span className="text-xl">🧾</span>
...
const SORTS = ["Lowest", "Highest", "Closest"];
const SORT_LABELS = {
  "Lowest":  "Lowest Points",
  "Highest": "Highest Points",
  "Closest": "Closest to Redeem",
};
...
export default function RewardsIndex() {
  ...
  const filtered = useMemo(() => {
    ...
    if (activeTab === "High Value")  list = list.filter((r) => r.points_required >= 300);

    const sortLabel = SORT_LABELS[sortBy] ?? sortBy;
    if (sortLabel === "Lowest Points")     list.sort((a, b) => a.points_required - b.points_required);
    if (sortLabel === "Highest Points")    list.sort((a, b) => b.points_required - a.points_required);
    if (sortLabel === "Closest to Redeem") list.sort((a, b) => (a.points_required - user.points) - (b.points_required - user.points));

    return list;
  }, [rewards, activeTab, sortBy, user.points]);

  const goToPage = useCallback((p) => {
    router.get(route("rewards.index"), { page: p }, {
      preserveScroll: true,
      preserveState:  true,
      only:           ["redemptions"],
    });
  ...
  const confirmRedeem = useCallback(() => {
    ...
    router.post(route("rewards.redeem", confirmReward.id), {}, {
      preserveScroll: true,
      onSuccess: (page) => {
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-slate-50">
          ...
          <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between gap-3">
            <div className="flex items-center gap-2.5 min-w-0">
              <Link
                ...
                className="text-white/70 hover:text-white font-semibold transition-colors flex-shrink-0 p-1 -ml-1 rounded-lg active:bg-white/10"
              >
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M15 19l-7-7 7-7" />
                </svg>
              </Link>
              <h1 className="text-white font-extrabold text-base tracking-tight truncate">Rewards</h1>
            </div>
            <div className="text-right flex-shrink-0">
              <p className="text-white/60 text-[10px] font-semibold uppercase tracking-wider leading-none mb-0.5">Your Points</p>
        ...
        {/* ── DESKTOP SECTION SWITCHER (hidden on mobile — using bottom nav instead) ── */}
        ...
        {/* ── MAIN CONTENT ── */}
        <div className={`max-w-4xl mx-auto px-4 py-6 space-y-5 pb-nav md:pb-6`}>
          ...
          {activeSection === "rewards" ? (
            <>
                ...
                <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-none -mx-4 px-4">
                  {TABS.map((tab) => (
                    <button
                      ...
                      className={`flex-shrink-0 px-4 py-2.5 rounded-xl text-xs font-bold transition-all duration-150 ${
                        activeTab === tab
                          ? "bg-[#1B1F5E] text-white shadow-sm"
              ...
              <div className="flex items-center gap-3 fade-up stagger-2">
                <span className="text-xs text-slate-400 font-semibold whitespace-nowrap flex-shrink-0">Sort:</span>
                <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-none">
                  {SORTS.map((s) => (
                    <button
                      key={s}
                      onClick={() => setSortBy(s)}
                      className={`flex-shrink-0 px-3 py-2 rounded-lg text-[11px] font-bold transition-all ${
                        sortBy === s
            ...
            /* ── HISTORY ── */
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 sm:p-6 fade-up stagger-1">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-sm font-extrabold text-[#1B1F5E] uppercase tracking-wider">My Redemptions</h2>
                <div className="flex items-center gap-2">
                  <span className="text-xs text-slate-400 font-semibold">{total} record{total !== 1 ? "s" : ""}</span>
                  {lastPage > 1 && (
              ...
              <RedemptionHistory redemptions={redemptionData} onView={setViewingRedemption} />
              <Pagination currentPage={currentPage} lastPage={lastPage} onPageChange={goToPage} />
            </div>
          )}
        </div>
      </div>
